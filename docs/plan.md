...
フェーズ A〜F はすべて実装済み。各フェーズの判断と実測値は [decisions.md](decisions.md) の D1〜D13 に記録した。
計画から変えた点・指示書どおりにならなかった点:

- タイムストレッチは R3(`high_quality=True`)ではなく R2 + 短い FFT 窓 + 比率ごとのずれ補正(D7)。R3 は比率によって拍位置が
  累積的にずれ、11章の ±3ms を満たせなかった
- セッション(`Session`)が Transport とレーンを持ち、`--no-audio` でもプロセス内のエンジンを使う形にした(D11)。インタプリタの
  コンストラクタが変わり、Scheduler からゲインオートメーションが消えたため、該当する既存テストは意図を保って書き換えた
  (コミットメッセージに理由)
- 12章「NullBackend で 10分間アンダーランゼロ」は開発機で厳密には未達: 外部要因と見られる約1.3秒の間に 3回。実デバイス
  (WASAPI)では 10分間ゼロ(D12)
- 区切りの直前 60ms 以内に打ったコマンドは次の区切りに回す(先読みとは別の「間に合わせるための余裕」。D13 の実測から)
- 聴感(クロスフェードの自然さ、ストレッチの質など)は開発者が聴けないため未確認。[listening-checklist.md](listening-checklist.md)
  に確認手順と、数値で確認済みの範囲を書いた。実際の音楽ファイルでの解析精度も未確認(合成音のみ)
