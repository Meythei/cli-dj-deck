...
def test_looping_lane_local_beat_always_in_range():
    ...
    lane.start_snippet(snippet, transport)
...
def test_check_warnings_bpm_drift():
    ...
    lane.start_snippet(make_snippet(bpm=128.0), transport)
...
def test_check_warnings_vocal_conflict():
    ...
    lane_a.start_snippet(make_snippet(role="vocal"), transport)
    lane_b.start_snippet(make_snippet(role="vocal"), transport)
...
def test_check_warnings_key_clash():
    transport = Transport(bpm=128.0)
    lane_a, lane_b = Lane("L1"), Lane("L2")
    lane_a.start_snippet(make_snippet(key="8A"), transport)
    lane_b.start_snippet(make_snippet(key="3A"), transport)
    warnings = check_warnings(lane_a, [lane_b], 128.0)
